...
class LiveLoadB3 extends ReportLoaderCalendar {
  ...
  async process(params: { dateRef: Date }): Promise<ILoadResult> {
    ...
    loadResults.push(await this.readLiveTS(new Date(), 'FRP0'));
    ...
    for await (const assets of (
      ...
      if (rollSeries && rollSeries.length > 0) {
        ...
        loadResults.push(
          await this.readLiveTS(new Date(), asset.concat(assetContract[0])),
        );

        for await (const rollSerie of rollSeries) {
          const contracts = rollSerie.match(/([F,G,H,J,K,M,N,Q,U,V,X,Z]\d\d)/g);
          loadResults.push(
            await this.readLiveTS(new Date(), asset.concat(contracts[0])),
          );
        }
      }
    }
  ...
  async readLiveTS(dateRef: Date, asset: string): Promise<ILoadResult> {
  ...
  private getAssetAuthSeriesCode(asset: string): number {
    ...
    return assetCode ? assetCode.authSeriesCode : undefined;
  }
}
