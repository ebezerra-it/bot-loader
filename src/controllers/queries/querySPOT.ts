...
interface ISpot {
  ...
  close: number;
  vwap: number;
  frp0: { today?: number; frp1d1?: number; calculated?: number };
  volume: number;
}
...
export default class QuerySPOT extends Query {
  ...
  public async calculateSpotToday(
    ...
    const qFRP0 = await this.queryFactory.runQuery(
      `SELECT (high + low)/2 as frp0 
      FROM "b3-assetsquotes" 
      WHERE datetime<$1 AND datetime::DATE = $1::DATE AND asset='FRP0' ORDER BY datetime DESC limit 1`,
      { date: dateRef.toJSDate() },
    );

    const dtRefD1 = await ReportLoaderCalendar.subTradeDays(
      this.queryFactory,
      dateRef,
      1,
      TCountryCode.BR,
    ...
    const qFRP1 = await this.queryFactory.runQuery(
      `SELECT "timestamp-open"::DATE date, (MAX(high) + LOW(low))/2 frp1 
      FROM "b3-ts-summary" 
      WHERE asset = 'FRP1' AND "timestamp-open"::DATE<$1::DATE 
      GROUP BY "timestamp-open"::DATE 
      ORDER BY date::DATE DESC 
      LIMIT 1`,
      {
        date: dtRefD1.toJSDate(),
      },
    );

    if (qFRP0 && qFRP0.length > 0 && !Number.isNaN(qFRP0[0].frp0))
      frp0 = Number(qFRP0[0].frp0);

    if (
      qFRP1 &&
      qFRP1.length > 0 &&
      !Number.isNaN(qFRP1[0].frp1) &&
      (
        ...
        .startOf('day')
        .toMillis() === dtRefD1.startOf('day').toMillis()
    )
      frp1 = Number(qFRP1[0].frp1);

    if (!frp0 && !frp1) {
      calculatedFrp0 = await QuerySPOT.calculateFRP0(
        this.queryFactory,
        dateRef,
      );
    }
    ...
    if (qSpot && qSpot.length > 0) {
      return {
        ...
        frp0: {
          today: frp0 ? +Number(frp0).toFixed(2) : undefined,
          frp1d1: frp1 ? +Number(frp1).toFixed(2) : undefined,
          calculated: calculatedFrp0
            ? +Number(calculatedFrp0).toFixed(2)
            : undefined,
        },
  ...
  public async calculateSpotForSettleDate(
    ...
    if (
      ...
      qFRP0 = await this.queryFactory.runQuery(
        `(SELECT "datetime"::DATE as date, (high + low)/2 as frp0 
        FROM "b3-assetsquotes" WHERE "datetime"::DATE = $2::DATE AND asset='FRP0'
  ...
  private static async calculateFRP0(
    ...
    const frp0 =
      ...
      Number(qPTAX[0].ptax) > 0
        ? +Number(
            Number(qPTAX[0].ptax) * (iDI1_close / iDDI_close - 1),
          ).toFixed(2)
        : undefined;
